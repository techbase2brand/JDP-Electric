...
import { blackColor, blueColor, greenColor, lightBlueColor, lightShadeBlue, orangeColor, whiteColor, yellowColor } from '../constants/Color';
...
const JobDetailsScreen = ({ navigation, route }) => {
    ...
    const renderItemUsed = ({ item, index }) => (
        <View style={[styles.tableRow, { backgroundColor: index % 2 === 0 ? whiteColor : '#F4FBFF' }]}>
    ...
    return (
        <View style={[flex, styles.container]}>
            <View style={[styles.headerContainer, justifyContentSpaceBetween, flexDirectionRow, alignItemsCenter]}>
                <View style={[flexDirectionRow, alignItemsCenter]}>
                    <TouchableOpacity onPress={() => navigation.goBack()}>
                        <Ionicons name="arrow-back-sharp" size={24} color={blackColor} />
                    </TouchableOpacity>
                    <Text style={[styles.headerText, { marginLeft: spacings.large }]}>#{id}</Text>
                </View>
                <View style={[flexDirectionRow, alignItemsCenter]}>
                    <TouchableOpacity>
                        <Ionicons name="stop-circle" size={24} color={blueColor} />
                    </TouchableOpacity>
                    <Text style={[styles.headerText, { marginLeft: spacings.small }]}>{currentTime}</Text>
                </View>
            </View>
            <View style={[styles.desContainer, flexDirectionRow]}>
                <View style={[{ width: "75%", padding: spacings.large }]}>
                    <Text style={[{ fontSize: style.fontSizeNormal.fontSize }]}>#65 Halsey Road, Encounter Bay, South Australia +61(08) 8298 0185 Australia 5211</Text>
                </View>
                <View style={[{ width: "25%", paddingHorizontal: spacings.large, height: hp(3), backgroundColor: yellowColor, borderBottomLeftRadius: 10 }, alignJustifyCenter]}>
                    <Text style={[{ fontSize: style.fontSizeNormal.fontSize, color: whiteColor }]}>{status}</Text>
                </View>
            </View>
            <Text style={[styles.headerText, { marginLeft: spacings.large }]}>Items Used</Text>
            ...
            <Text style={[styles.headerText, { marginLeft: spacings.large }]}>Team</Text>
            <View style={styles.tableHeader}>
                <Text style={styles.headerText}>Sr No.</Text>
